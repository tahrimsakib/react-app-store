          ...
          {firstEight.map((singleApp) => {
            ...
            return (
              <Link key={id} to={`app/appdetails/${id}`}>
                <div
                  key={id}
                  className="bg-white p-3 shadow-xl rounded-lg hover:scale-[1.02] w-fit duration-200 transition"
                >
                  <img
                    src={image}
                    alt={title}
                  ...
                  <div className="my-2.5 flex justify-between items-center text-[14px]">
                    ...
                    <p className="flex justify-center gap-1.5 items-center py-0.5 bg-[#fff0e1] text-orange-500 font-semibold px-2 rounded-[4px]">
